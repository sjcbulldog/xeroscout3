    ...
    public loadStatboticsEventData(key: string, sb: StatBotics, callback?: (result: string) => void) : Promise<void> {
        let ret: Promise<void> = new Promise<void>(async (resolve, reject) => {
            try {
                ...
                const validStats = stats.filter((t: any) => t && typeof t.team === 'number') ;

                if (validStats.length === 0) {
                    if (callback) {
                        callback('No Statbotics event data found; requesting team EPA for the year instead.<br>') ;
                    }
                    resolve() ;
                    return ;
                }
                ...
                await this.data_mgr_!.processStatboticsEventData(validStats) ;
                if (callback) {
