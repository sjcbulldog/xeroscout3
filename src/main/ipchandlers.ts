...
import { IPCCheckDBViewFormula, IPCDataSet, IPCGetTeamsOptions, IPCGraphConfig, IPCNamedDataValue, IPCPickListConfig, IPCProjColumnsConfig, IPCPromptStringRequest, IPCPromptStringResponse, IPCTeamInfo, IPCMatchPredictorRequest } from "../shared/ipc";
import { SCCoachCentralBaseApp } from "./apps/sccoachcentralbase";
import { IPCAppType, IPCMatchViewConfig } from "../shared/ipc";
...
export async function getMatchPredictorData(cmd: string, ...args: any[]) {
    if (isCentralOrCoachType()) {
        ...
        if (args.length === 1 && typeof args[0] === 'object') {
            central.sendMatchPredictorData(args[0] as IPCMatchPredictorRequest) ;
        } else {
            scappbase!.logger_.error({ message: 'renderer -> main invalid args', args: {cmd: cmd, cmdargs: args}});
        }
    }
...
export async function getTeamList(cmd: string, ...args: any[]) {
    if (isCentralOrCoachType()) {
        ...
        let central : SCCentral = scappbase as SCCentral ;
        if (args.length === 1 && typeof args[0] === 'object') {
            central.getTeamList(args[0] as IPCGetTeamsOptions) ;
        }
    } 
}
...
export async function getPicklistConfigs(cmd: string, ...args: any[]) {
    if (isCentralOrCoachType()) {
        scappbase!.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});
        let central : SCCentral = scappbase as SCCentral ;
        if (args.length === 0) {
...
export async function getPlayoffStatus(cmd: string, ...args: any[]) {
    if (isCentralOrCoachType()) {
        scappbase!.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});
        let central : SCCentral = scappbase as SCCentral ;
        if (args.length === 0) {       
...
export async function getMatchConfigs(cmd: string, ...args: any[]) {    
    if (scappbase && scappbase.applicationType === 'central') {    
        let central: SCCentral = scappbase as SCCentral;    
        if (args.length === 0) {    
...
export async function updateMatchConfig(cmd: string, ...args: any[]) {  
    if (scappbase && scappbase.applicationType === 'central') {  
        scappbase.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});  
        let central : SCCentral = scappbase as SCCentral ;  
  
        if (args.length === 1 && typeof args[0] === 'object') {  
            central.updateMatchConfig(args[0] as IPCMatchViewConfig) ;  
        }  
        else {  
            scappbase.logger_.error({ message: 'renderer -> main invalid args', args: {cmd: cmd, cmdargs: args}});  
        }  
...
export async function updateMatchCurrent(cmd: string, ...args: any[]) {  
    if (scappbase && scappbase.applicationType === 'central') {  
        scappbase.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});  
        let central : SCCentral = scappbase as SCCentral ;  
  
        if (args.length === 1 && typeof args[0] === 'string') {  
            central.updateMatchCurrent(args[0] as string) ;  
        }  
        else {  
            scappbase.logger_.error({ message: 'renderer -> main invalid args', args: {cmd: cmd, cmdargs: args}});  
        }  
...
export async function deleteMatchConfig(cmd: string, ...args: any[]) {  
    if (scappbase && scappbase.applicationType === 'central') {  
        scappbase.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});  
        let central : SCCentral = scappbase as SCCentral ;  
  
        if (args.length === 1 && typeof args[0] === 'string') {  
            central.deleteMatchConfig(args[0] as string) ;  
        }  
        else {  
            scappbase.logger_.error({ message: 'renderer -> main invalid args', args: {cmd: cmd, cmdargs: args}});  
        }  
...
export async function getMatchConfigData(cmd: string, ...args: any[]) {  
    if (scappbase && scappbase.applicationType === 'central') {  
        scappbase.logger_.silly({ message: 'renderer -> main', args: {cmd: cmd, cmdargs: args}});  
        let central : SCCentral = scappbase as SCCentral ;  
        if (args.length === 1 && typeof args[0] === 'object') {         
            ...
            if (obj.hasOwnProperty('name') && obj.hasOwnProperty('description') && obj.hasOwnProperty('dataset') && obj.hasOwnProperty('fields')) {  
                central.getMatchData(obj as IPCMatchViewConfig) ;  
            }  
            else {  
                scappbase.logger_.error({ message: 'renderer -> main invalid args', args: {cmd: cmd, cmdargs: args}});  
            }  
